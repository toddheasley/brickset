...
struct URLSessionTests {
    ...
    @Test(.enabled(if: isTestable)) func setCollection() async throws {
        try await URLSession.shared.login(username, password: password)
        try await URLSession.shared.setCollection(SetCollection(own: true, notes: "Favorite LEGO set, all time", rating: 5), set: 33645)
    }
    
    @Test(.enabled(if: isTestable)) func getUserNotes() async throws {
        try await URLSession.shared.login(username, password: password)
        let userNotes: ([UserNotes], Int?) = try await URLSession.shared.getUserNotes()
    ...
    @Test(.enabled(if: isTestable)) func getMinifigCollection() async throws {
        try await URLSession.shared.login(username, password: password)
        let minifigCollection: ([MinifigCollection], Int?) = try await URLSession.shared.getMinifigCollection(GetMinifigCollection(query: "train"))
    ...
    @Test(.enabled(if: isTestable)) func setMinifigCollection() async throws {
        try await URLSession.shared.login(username, password: password)
        try await URLSession.shared.setMinifigCollection(SetMinifigCollection(own: true), set: "col445")
        try await URLSession.shared.setMinifigCollection(SetMinifigCollection(notes: "Favorite fig from the last series"), set: "col445")
    }
    
    @Test(.enabled(if: isTestable)) func getUserMinifigNotes() async throws {
        try await URLSession.shared.login(username, password: password)
        try await URLSession.shared.setMinifigCollection(SetMinifigCollection(notes: "Favorite fig from the last series"), set: "col445")
        let userMinifigNotes: ([UserMinifigNotes], Int?) = try await URLSession.shared.getUserMinifigNotes()
